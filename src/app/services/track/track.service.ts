...
export async function searchTracks(
    ...
    const {
        limit,
        includeTypes = new Set([TrackSearchTypesEnum.TRACK])
    } = options;
...
export const searchLyrics = memoize(
    async (trackName: string, artist: string) => {
        const params = {
            ['q_track']: trackName,
            ['q_artist']: artist,
            ...musixmatchParams
        ...
        const response: any = await httpMusixmatch.get('matcher.lyrics.get', {
            params: params
        });
...
export const getTopTracks = memoize(
    async (limit?: number): Promise<Track[]> => {
        let tracks = [];
        const params = { limit };

        tracks = await httpSpotify.get(
...
export const getTracks = memoize(
    async (ids: string[]): Promise<Track[]> => {
