...
export const SavedTracksListContainer = ({ selectedTrack, onTrackSelected }: SavedTracksListContainerProps) => {
  const [savedTracksPage, setTracksPage] = React.useState<PagingObject<SavedTrack>>();
  const [isAdditionalTracksLoading, setIsAdditionalTracksLoading] = React.useState<boolean>();
  ...
  const fetchNext = React.useCallback(() => {
    if (savedTracksPageRef.current && savedTracksPageRef.current.items.length !== savedTracksPageRef.current.total) {
      setIsAdditionalTracksLoading(true);
      libraryService.getUserTracks({ nextPage: savedTracksPageRef.current.next }).then((tracksPage) => {
        const newTracks = [...savedTracksPageRef.current.items, ...tracksPage.items]
  ...
  return (
    <SavedTracksListComponent
      savedTracks={savedTracksPage && savedTracksPage.items}
